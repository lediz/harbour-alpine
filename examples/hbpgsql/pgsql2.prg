...
LOCAL conn, res, aTemp, x, y, pFile
   LOCAL cDb := "visionbanco"
   LOCAL cUser := "postgres"
   LOCAL cPass := "secret"

   CLS

   conn := PQsetdbLogin( "10.9.100.111", "5432", NIL, NIL, cDb, cUser, cPass )
   ? PQdb( conn ), PQuser( conn ), PQpass( conn ), PQhost( conn ), PQport( conn ), PQtty( conn ), PQoptions( conn )

   conn := PQconnectdb( "dbname = " + cDb + " host = 10.9.100.111 user = " + cUser + " password = " + cPass + " port = 5432" )
